...
## @deprecated: Use [TileMapDual] instead.
...
const _NEIGHBORS := {
	...
	Direction.TOP_RIGHT: TileSet.CellNeighbor.CELL_NEIGHBOR_TOP_RIGHT_CORNER,
	Direction.BOTTOM_LEFT: TileSet.CellNeighbor.CELL_NEIGHBOR_BOTTOM_LEFT_CORNER,
...
const _NEIGHBORS_ISOMETRIC := {
	...
	Direction.TOP_RIGHT: TileSet.CellNeighbor.CELL_NEIGHBOR_RIGHT_CORNER,
	Direction.BOTTOM_LEFT: TileSet.CellNeighbor.CELL_NEIGHBOR_LEFT_CORNER,
...
func update_tile(world_cell: Vector2i, recurse: bool = true) -> void:
	...
	var _actual_neighbors = _NEIGHBORS_ISOMETRIC if is_isometric else _NEIGHBORS
	var _top_left = world_cell
	var _low_left = display_tilemap.get_neighbor_cell(
		world_cell,
		_actual_neighbors[Direction.BOTTOM],
	)
	var _top_right = display_tilemap.get_neighbor_cell(
		world_cell,
		_actual_neighbors[Direction.RIGHT],
	)
	var _low_right = display_tilemap.get_neighbor_cell(
		world_cell,
		_actual_neighbors[Direction.BOTTOM_RIGHT],
	)
	...
	# if atlas id is -1 the tile is empty, so to have a good rendering
	# we need to update surroundings
	if (_atlas_id == -1):
		update_tile(
			self.get_neighbor_cell(
				world_cell,
				_actual_neighbors[Direction.LEFT],
			),
			false,
		)
		update_tile(
			self.get_neighbor_cell(
				world_cell,
				_actual_neighbors[Direction.TOP_LEFT],
			),
			false,
		)
		update_tile(
			self.get_neighbor_cell(
				world_cell,
				_actual_neighbors[Direction.TOP],
			),
			false,
		)
		update_tile(
			self.get_neighbor_cell(
				world_cell,
				_actual_neighbors[Direction.TOP_RIGHT],
			),
			false,
		)
		update_tile(
		...
		update_tile(
			self.get_neighbor_cell(
				world_cell,
				_actual_neighbors[Direction.BOTTOM_LEFT],
			),
			false,
		)
...
func _update_tileset() -> void:
	if display_tilemap == null:
		update_full_tileset()
		return
	if display_tilemap.tile_set != self.tile_set: # TO-DO: merge with the above
		update_full_tileset()
		return
	if _tile_size != self.tile_set.tile_size or _tile_shape != self.tile_set.tile_shape:
		update_geometry()
		return
	...
	var _changed_cells: Dictionary = exclude_dicts(
		_emptied_cells,
		_new_emptied_cells,
	).merged(exclude_dicts(_filled_cells, _new_filled_cells))
...
func _update_displayed_tile(_display_cell: Vector2i) -> void:
	...
	var _actual_neighbors = _NEIGHBORS_ISOMETRIC if is_isometric else _NEIGHBORS
	var _top_left = display_tilemap.get_neighbor_cell(
		_display_cell,
		_actual_neighbors[Direction.TOP_LEFT],
	)
	var _low_left = display_tilemap.get_neighbor_cell(
